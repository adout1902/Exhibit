...
for i in templates:
    if templates[i]["title"] == name:
        templateData = json.dumps(templates[i])
        break
...
function createHdrFtr() {
    ...
    tray.style.backgroundColor = "lightBlue";
    ...
    for(var i = 1; i <= 11; i++) {
        ...
        t.id = "thumbnail-"+i;
        // t.textContent = "thumbnail-"+i;
        t.className = "thumbs";
        t.style.textAlign = "center";
        t.style.backgroundColor = "lightPink";
        // t.style.display = "none";
        t.style.border = "2px solid hotPink";
        t.style.padding = "10px";
        t.style.margin = "10px";

        var s = document.createElement("span");
        s.className = "span-helper";
...
function test(num) {
    console.log("Test function called by button " + num);
}
...
function createTitle() {
    ...
    CKEDITOR.replace("title-editor", {
        width: titleArea.style.width,
        height: titleArea.style.height,
        toolbarGroups: [
...
/* ========== TEXTBOX CODE ========== */
function createTextEditors(data) {
    ...
    for(var i = 1; i <= noTextboxes; i++) {
        ...
        CKEDITOR.replace(textEditor.id, {
...
function addTextArea() {
    return `<div id=textId><textarea id=texteditor></textarea></div>`
}
...
function addImage() {
    return `
        <div class="img-divs" id="img-div">
            ...
            <form id=input style="display:none" action=ajax.py method=post>
...
function uploadImage() {
    ...
    o.style.zIndex = "9999";
    o.style.border = "5px solid black";
    ...
    $.fn.ajaxForm = function(options) {
        ...
        if (!$iframe.length) {
            $iframe = $('<iframe name=' + iframeName + ' style="display:block">').appendTo('body');
        }
...
function print_text(this_id) {
    console.log("Button "+ this_id +" is working !");
}
