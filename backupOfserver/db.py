...
def addTemplate(title,creators,img,templateContents):
    ...
    with open("/home/ubuntu/json/TemplatesDB.json", 'w') as f:
        portalocker.lock(f, portalocker.LOCK_EX)
        json.dump(data,f,indent=4)
    response["message"] = "added template"
...
def updateTemplate(id,img,templateContents):
    ...
    with open("/home/ubuntu/json/TemplatesDB.json", 'w') as f:
        portalocker.lock(f, portalocker.LOCK_EX)
        json.dump(data,f,indent=4)
        response["message"] = "updated template"
        print(json.dumps(response))
...
def addComment(id,name,text):
    ...
    comments.append({"username":name,"text":text})
    with open("/home/ubuntu/json/TemplatesDB.json", 'w') as f:
        portalocker.lock(f, portalocker.LOCK_EX)
        json.dump(data,f,indent=4)
    response["message"] = "added comment"
    print(json.dumps(response))
...
if action == "add":
    ...
    contents = json.loads(fs.getvalue("contents"))
    templateImg = fs.getvalue("templateImg")
    updateTemplate(str(templateID),templateImg,contents)
    ...
    addComment(str(templateID),name,text)
